import {
  ...
  showEndOfTrick,
  showLastPlayerMove
} from './utils/determine-domino-locations'
...
const ShowDominoes = ({ globals, windowHeight, windowWidth, lastMessage, stagedDomino, setStagedDomino }: Props) => {
  ...
  React.useEffect(() => {
    if (isEndOfTrick) {
      ...
      setTimeout(
        () => { showEndOfTrick(windowWidth, windowHeight, trickDominoSize, dominoes, stagedDomino as DominoObj, setStagedDomino, otherStagedDominoes, setOtherStagedDominoes, winningTeam, teamTricks, setTeamTricks, moveDominoes) },
        1000
      )
